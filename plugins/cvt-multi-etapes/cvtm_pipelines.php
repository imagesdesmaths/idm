 ...
 * @return array
 */
function cvtm_recuperer_post_precedents($form){
 ...
 * @return <type> 
 */
function cvtm_formulaire_charger($flux){
	...
	if (isset($flux['data']['_etapes'])){
		...
		$flux['data'] = cvtm_sauver_post($form, $je_suis_poste, $flux['data']);
		#var_dump($flux['data']);
	}
	return $flux;
}
 ...
 * @return array
 */
function cvtm_formulaire_verifier($flux){
	#var_dump('Pipe verifier');
	
	if ($form = $flux['args']['form']
	  AND ($e = cvtm_recuperer_post_precedents($form))!==false){
