...
function formulaires_relecteurs_gestion_traiter () {
  ...
  foreach ($candidats as $row) {
    ...
    if ($row["role"] == "candidat") {
      ...
      if ($decision == "oui") {
        sql_updateq ("spip_idm_relecteurs", array("role"=>"relecteur", "categorie"=>"nouveau", "math" => supprimer_tags(_request("math_$id"))), "id_auteur=$id");
        sql_insertq ("spip_idm_relecture", array("id_auteur"=>intval($id), "action"=>"born"));
      }
