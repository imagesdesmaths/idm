...
function balise_THICKBOX_GALLERY($p) {
   return calculer_balise_dynamique($p, THICKBOX_IMG, array());
}

function balise_THICKBOX_GALLERY_dyn($text, $url, $title='', $text, $_width=false, $_height=false, $mode='', $id='') {
	return balise_thickbox_dyn('gallery', $_url, $title, $text, $_width, $_height, $mode);
}
?>